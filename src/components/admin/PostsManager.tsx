...
import { FileText, Plus, Search, Loader2, Trash2, Edit3, AlertCircle, Save, ChevronUp, ChevronDown, Check, X, CalendarClock, Send } from 'lucide-react';
...
export default function PostsManager() {
        ...
        try {
            ...
            if (postsRes.status === 'fulfilled') {
                ...
                if (allCategories.size > 0) {
                    ...
                    const uniqueCategories = Array.from(allCategories).filter((cat: any) => {
    ...
    const requestIndexing = async (post: any) => {
        if (post.draft) {
            triggerToast('Publique o post antes de solicitar indexacao.', 'info');
            return;
        }

        const url = new URL(`/${post.slug}/`, window.location.origin).toString();
        setIndexingSlug(post.slug);
        triggerToast('Enviando URL para a Indexing API...', 'progress', 35);

        try {
            const res = await fetch('/api/admin/plugins/search-console/request-indexing', {
                method: 'POST',
            ...
            if (!res.ok || !data.success) {
                throw new Error(data.message || 'Erro ao solicitar indexacao.');
            }
            triggerToast('Solicitacao de indexacao enviada ao Google!', 'success', 100, url);
        } catch (err: any) {
            triggerToast(`Erro: ${err.message}`, 'error');
        } finally {
                                        ...
                                        <tr className="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td className="py-4 px-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => requestIndexing(post)}
                                                        disabled={post.draft || indexingSlug === post.slug}
                                                        title={post.draft ? 'Publique o post antes de solicitar indexacao' : 'Solicitar indexacao'}
                                                    ...
                                                    >
                                                        {indexingSlug === post.slug ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />}
                                                    </button>
