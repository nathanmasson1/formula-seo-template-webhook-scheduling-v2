...
import { Save, AlertCircle, Loader2, ArrowLeft, Image as ImageIcon, Eye, Edit3, CalendarClock, Bold, Italic, List, ListOrdered, Link, Heading2, Heading3, Eraser } from 'lucide-react';
...
function SimpleHtmlEditor({ value, onChange }: { value: string; onChange: (value: string) => void }) {
    ...
    const toolbarButtonClass = 'inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 hover:border-violet-300 hover:bg-violet-50 hover:text-violet-700 transition-colors';

    return (
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
            <div className="flex flex-wrap gap-2 border-b border-slate-200 bg-slate-50 p-3">
                <button type="button" title="Título H2" className={toolbarButtonClass} onClick={() => runCommand('formatBlock', 'h2')}><Heading2 className="w-4 h-4" /></button>
                <button type="button" title="Título H3" className={toolbarButtonClass} onClick={() => runCommand('formatBlock', 'h3')}><Heading3 className="w-4 h-4" /></button>
                <button type="button" title="Negrito" className={toolbarButtonClass} onClick={() => runCommand('bold')}><Bold className="w-4 h-4" /></button>
                <button type="button" title="Itálico" className={toolbarButtonClass} onClick={() => runCommand('italic')}><Italic className="w-4 h-4" /></button>
                <button type="button" title="Lista" className={toolbarButtonClass} onClick={() => runCommand('insertUnorderedList')}><List className="w-4 h-4" /></button>
                <button type="button" title="Lista numerada" className={toolbarButtonClass} onClick={() => runCommand('insertOrderedList')}><ListOrdered className="w-4 h-4" /></button>
                <button type="button" title="Link" className={toolbarButtonClass} onClick={addLink}><Link className="w-4 h-4" /></button>
                <button type="button" title="Limpar formatação" className={toolbarButtonClass} onClick={() => runCommand('removeFormat')}><Eraser className="w-4 h-4" /></button>
            </div>
            <div
                ref={editorRef}
                contentEditable
                suppressContentEditableWarning
                className="prose prose-slate max-w-none min-h-[360px] p-6 text-slate-800 focus:outline-none"
                data-placeholder="Escreva o conteúdo do artigo..."
                onInput={sync}
                onBlur={sync}
