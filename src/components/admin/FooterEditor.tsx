...
import { Save, Loader2, AlertCircle, Plus, Trash2, Megaphone, Info, Share2, Columns, FileText, ChevronDown, ChevronUp, MapPin } from 'lucide-react';
...
type FooterConfig = {
    ...
    social: Social[];
    columns: Column[];
    nap: { heading: string; name: string; address: string; phones: string[]; email: string };
    copyright: string;
    disclaimer: string;
};
...
export default function FooterEditor() {
    ...
    useEffect(() => {
        githubApi('read', 'src/data/footer.json')
            .then(data => {
                ...
                setConfig({
                    cta: { title: '', titleAccent: '', text: '', ...(parsed.cta || {}) },
                    about: parsed.about || '',
    ...
    return (
        <div className="space-y-6 pb-32">
            ...
            {/* NAP */}
            <SectionCard title="NAP / Contato" icon={<MapPin className="w-5 h-5 text-violet-500" />} defaultOpen={true}>
                <div className="space-y-4">
                    <p className="text-xs text-slate-500">Cadastre os dados de Nome, Endereço e Telefone exibidos como coluna no rodapé.</p>
                    <div>
                        <label className={labelClass}>Título da coluna</label>
                        <input className={inputClass} value={config.nap.heading || ''} onChange={e => set('nap.heading', e.target.value)} placeholder="Contato" />
                    </div>
                    <div>
                        <label className={labelClass}>Nome / Empresa</label>
                        <input className={inputClass} value={config.nap.name || ''} onChange={e => set('nap.name', e.target.value)} placeholder="Escola de SEO - Por Luciano Arthur" />
                    </div>
                    <div>
                        <label className={labelClass}>Endereço</label>
                        <input className={inputClass} value={config.nap.address || ''} onChange={e => set('nap.address', e.target.value)} placeholder="RUA A N1 Blumenau SC" />
                    </div>
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            <label className={labelClass}>Telefones</label>
                            <button onClick={() => set('nap.phones', [...(config.nap.phones || []), ''])} className="text-xs flex items-center gap-1 text-indigo-600 hover:text-indigo-700 font-bold">
                                <Plus className="w-3 h-3" /> Adicionar telefone
                            </button>
                        </div>
                        {(config.nap.phones || []).map((phone, i) => (
                            <div key={i} className="grid grid-cols-[1fr_auto] gap-2">
                                <input className={inputClass} value={phone} onChange={e => {
                                    const phones = [...(config.nap.phones || [])];
                                    phones[i] = e.target.value;
                                    set('nap.phones', phones);
                                }} placeholder="47 3091-1019" />
                                <button onClick={() => set('nap.phones', (config.nap.phones || []).filter((_, x) => x !== i))} className="text-red-500 hover:bg-red-50 rounded-lg px-3">
                    ...
                    <div>
                        ...
                        <input className={inputClass} value={config.nap.email || ''} onChange={e => set('nap.email', e.target.value)} placeholder="suporte@escoladeseo.com" />
                    </div>
                </div>
            </SectionCard>
