...
type DashboardStats = { totalPosts: number; publishedPosts: number; draftPosts: number; totalAuthors: number };

export default function CmsDashboard({ initialStats }: { initialStats?: DashboardStats }) {
    const [loading, setLoading] = useState(!initialStats);
    const [stats, setStats] = useState<DashboardStats>(initialStats || { totalPosts: 0, publishedPosts: 0, draftPosts: 0, totalAuthors: 0 });
    ...
    useEffect(() => {
        ...
        const withTimeout = <T,>(promise: Promise<T>, ms = 6000): Promise<T> => {
            return Promise.race([
                promise,
                new Promise<T>((_, reject) => window.setTimeout(() => reject(new Error('Tempo limite ao carregar estatísticas.')), ms)),
            ]);
        };
        ...
        const fetchStats = async () => {
            try {
                ...
                try {
                    const postData = await withTimeout(githubApi('list', 'src/content/blog'));
                    if (Array.isArray(postData.data)) {
                        const mds = postData.data.filter((f: any) => f.name.endsWith('.md'));
                        tPosts = mds.length;
                        pubPosts = mds.length;
                        const draftChecks = mds.slice(0, 30).map(async (f: any) => {
                            try {
                                const fileData = await githubApi('read', f.path);
                                const text = fileData.content;
                                const draftMatch = text.match(/draft:\s*(true|false)/i);
                                return draftMatch && draftMatch[1].toLowerCase() === 'true';
                            } catch { return false; }
                        });
                        const draftResults = await withTimeout(Promise.all(draftChecks), 6000).catch(() => []);
                        drfPosts = draftResults.filter(Boolean).length;
                        pubPosts = Math.max(0, tPosts - drfPosts);
                    }
                } catch { /* ignora */ }

                let tAuthors = 0;
                try {
                    const data = await withTimeout(githubApi('read', 'src/data/authors.json'));
                    const parsed = JSON.parse(data?.content || "{}");
                    if (Array.isArray(parsed)) tAuthors = parsed.length;
                    else if (Array.isArray(parsed.items)) tAuthors = parsed.items.length;
                    else if (parsed.authors && Array.isArray(parsed.authors)) tAuthors = parsed.authors.length;
                } catch { /* ignora */ }
