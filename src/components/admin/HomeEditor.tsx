...
type ProofBrand = { title: string; description: string };
        ...
        githubApi('read', 'src/data/home.json')
            .then(data => {
                ...
                parsed.socialProof = {
                    ...
                    brands: (parsed.socialProof?.brands || []).map((brand: any) => (
                        typeof brand === 'string' ? { title: brand, description: '' } : brand
                    )),
                };
                parsed.linkBuilding = {
                    label: parsed.linkBuilding?.label || 'Autoridade que sustenta o crescimento',
                    title: parsed.linkBuilding?.title || 'Link building transforma conteúdo em presença real no Google',
                    titleAccent: parsed.linkBuilding?.titleAccent || 'presença real',
                    text: parsed.linkBuilding?.text || '',
                    bullets: parsed.linkBuilding?.bullets || [],
                    ctaText: parsed.linkBuilding?.ctaText || 'Conhecer estratégia de backlinks',
                    ctaHref: parsed.linkBuilding?.ctaHref || '/backlinks',
                    metrics: parsed.linkBuilding?.metrics || [],
                };
                setConfig(parsed);
                setFileSha(data.sha);
            })
        ...
        <div className="space-y-6 pb-32">
            ...
            <SectionCard title="Social Proof" icon={<Star className="w-5 h-5 text-yellow-500" />}>
                <div className="space-y-4">
                    ...
                    <div>
                        <label className={labelClass}>Título dos Serviços</label>
                        <input
                            className={inputClass}
                            value={config.socialProof.brandLabel || ''}
                            onChange={e => set('socialProof.brandLabel', e.target.value)}
                            placeholder="Serviços integrados para crescimento orgânico"
                        />
                    </div>
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className={labelClass}>Serviços / Descrições</label>
                            <button
                                onClick={() => set('socialProof.brands', [...config.socialProof.brands, { title: '', description: '' }])}
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            {config.socialProof.brands.map((brand, i) => (
                                <div key={i} className="grid grid-cols-[1fr_2fr_auto] gap-2 border border-slate-200 rounded-xl p-3">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            value={brand.title || ''}
                                            onChange={e => {
                                                const b = [...config.socialProof.brands];
                                                b[i] = { ...b[i], title: e.target.value };
                                                set('socialProof.brands', b);
                                            }}
                                            placeholder="SEO Técnico"
                                        />
                                    </div>
                                    <div>
                                        ...
                                        <input
                                            ...
                                            placeholder="Ajustes técnicos para melhorar indexação, velocidade e estrutura."
                                        />
                                    </div>
                                    <button onClick={() => set('socialProof.brands', config.socialProof.brands.filter((_, j) => j !== i))} className="self-end p-2 text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </div>
            ...
            {/* LINK BUILDING */}
            <SectionCard title="Link Building" icon={<LinkIcon className="w-5 h-5 text-violet-500" />}>
                <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-4">
                        <div>
                            <label className={labelClass}>Etiqueta</label>
                            <input className={inputClass} value={config.linkBuilding.label} onChange={e => set('linkBuilding.label', e.target.value)} />
                        </div>
                        <div>
                            <label className={labelClass}>Texto destacado</label>
                            <input className={inputClass} value={config.linkBuilding.titleAccent} onChange={e => set('linkBuilding.titleAccent', e.target.value)} placeholder="presença real" />
                        </div>
                        <div>
                            <label className={labelClass}>Link do botão</label>
                            <input className={inputClass} value={config.linkBuilding.ctaHref} onChange={e => set('linkBuilding.ctaHref', e.target.value)} placeholder="/backlinks" />
                        </div>
                    </div>
                    <div>
                        <label className={labelClass}>Título</label>
                        <input className={inputClass} value={config.linkBuilding.title} onChange={e => set('linkBuilding.title', e.target.value)} />
                    </div>
                    <div>
                        <label className={labelClass}>Texto</label>
                        <textarea className={inputClass} rows={4} value={config.linkBuilding.text} onChange={e => set('linkBuilding.text', e.target.value)} />
                    </div>
                    <div>
                        <label className={labelClass}>Texto do botão</label>
                        <input className={inputClass} value={config.linkBuilding.ctaText} onChange={e => set('linkBuilding.ctaText', e.target.value)} />
                    </div>
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className={labelClass}>Pontos de apoio</label>
                            <button onClick={() => set('linkBuilding.bullets', [...config.linkBuilding.bullets, ''])} className="text-xs text-indigo-600 font-bold flex items-center gap-1"><Plus className="w-3 h-3" /> Adicionar Ponto</button>
                        </div>
                        <div className="space-y-2">
                            {config.linkBuilding.bullets.map((bullet, i) => (
                                <div key={i} className="grid grid-cols-[1fr_auto] gap-2">
                                    <input
                                        className={inputClass}
                                        value={bullet}
                                        onChange={e => {
                                            const b = [...config.linkBuilding.bullets];
                                            b[i] = e.target.value;
                                            set('linkBuilding.bullets', b);
                                        }}
                                    />
                                    <button onClick={() => set('linkBuilding.bullets', config.linkBuilding.bullets.filter((_, j) => j !== i))} className="p-2 text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </div>
                    <div>
                        <div className="flex items-center justify-between mb-2">
                            <label className={labelClass}>Etapas do painel visual</label>
                            <button onClick={() => set('linkBuilding.metrics', [...config.linkBuilding.metrics, { number: '', label: '' }])} className="text-xs text-indigo-600 font-bold flex items-center gap-1"><Plus className="w-3 h-3" /> Adicionar Etapa</button>
                        </div>
                        <div className="space-y-2">
                            {config.linkBuilding.metrics.map((metric, i) => (
                                <div key={i} className="grid grid-cols-[0.4fr_1fr_auto] gap-2">
                                    <input
                                        className={inputClass}
                                        value={metric.number}
                                        onChange={e => {
                                            const m = [...config.linkBuilding.metrics];
                                            m[i] = { ...m[i], number: e.target.value };
                                            set('linkBuilding.metrics', m);
                                        }}
                                        placeholder="01"
                                    />
                                    <input
                                    ...
                                    />
                                    <button onClick={() => set('linkBuilding.metrics', config.linkBuilding.metrics.filter((_, j) => j !== i))} className="p-2 text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                                </div>
                            ))}
                        </div>
                    </div>
