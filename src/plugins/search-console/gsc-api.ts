...
function createJWT(credentials: GSCCredentials, scope = 'https://www.googleapis.com/auth/webmasters.readonly'): string {
...
export async function getAccessToken(credentials: GSCCredentials, scope?: string): Promise<string> {
...
export async function requestUrlIndexing(url: string, credentials: GSCCredentials): Promise<any> {
    ...
    const payload = await res.json().catch(async () => ({ error: { message: await res.text() } }));
    if (!res.ok) {
        throw new Error(payload?.error?.message || `Erro Indexing API (${res.status})`);
    }
    return payload;
}
